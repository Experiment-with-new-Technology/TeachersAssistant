...
public class Editlab extends Activity {
	SQLiteDatabase db;
	String w="";
	Button editclass;
    @Override
				...
				public void onClick(View v) {
			        ...
			        c.close(); 
			           ...
			           values.put("number", jnumber.getText().toString());
			        	   if(((db.update("lab", values, "title='"+jtitle.getText().toString()+"'", null))!=-1)&&!w.equals(""))
			           {
			        		   Toast.makeText(Editlab.this, "Record Successfully updated", 2000).show();
			           
			           }
			        	   else
			        		   Toast.makeText(Editlab.this, "update Error,Please input correct information and try again", 2000).show();
