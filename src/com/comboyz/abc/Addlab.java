           ...
           // db.execSQL(ssql);
            ...
            insertclass.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
			        ...
			        db.execSQL("Create Table if not exists '"+jtitle.getText().toString()+"'(serial INTEGER PRIMARY KEY AUTOINCREMENT,id text,name text,attendence integer,labperformance real,assignment real,labfinal real)");
			        
			        Cursor c=db.rawQuery("SELECT * FROM lab where title='"+jtitle.getText().toString()+"'",null);
			        c.moveToFirst();
			        while(!c.isAfterLast())
			        {
			        ...
			        if(jtitle.getText().toString().equals(aa))
			        	Toast.makeText(Addlab.this, "This Course is already token", 2000).show();
			        else{
	           ContentValues values=new ContentValues();
			           values.put("title", jtitle.getText().toString());
			           values.put("number", jnumber.getText().toString());
			           
			           if((db.insert("lab", null, values))!=-1)
			           {
			        	   File dir = new File("/sdcard/TeachersAssistant/"+jtitle.getText().toString()); 
			               if (!dir.exists() ){
			                    if (!dir.mkdir()){ 
			                  	  Toast.makeText(Addlab.this, "Unable to create directory", 1000).show();
			                       return; 
			                   } 
			               } 
			        	   Toast.makeText(Addlab.this, "Successfully "+jtitle.getText().toString()+" Inserted", 2000).show();
			           }
			           else
			           {
			           Toast.makeText(Addlab.this, "Insert Error,Please input correct information and try again", 2000).show();
			           }
			           jtitle.setText("");
			           jnumber.setText("");
